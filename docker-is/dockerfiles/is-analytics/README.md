...
##### IMPORTANT! Notice in version 5.4.1 the IS and the IS Analytics are both in the same package.
#####            So you might want to copy the same folder ./files/wso2is-5.4.1 into the analytics folder with the Dockerfile with the following name ./files/wso2is-analytics-5.4.1
- Once both JDK and WSO2 Identity Analytics Server distributions are extracted it may look as follows:
  ...
  ```
- Download [MySQL Connector/J](https://dev.mysql.com/downloads/connector/j/) v5.1.34 and then copy that to `<ANALYTICS_DOCKERFILE_HOME>/files` folder
